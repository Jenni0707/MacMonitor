...
public class FileupdataUtil extends Thread {// implements Runnable{
	...
	private void readname() {
		...
		if (ftp == null) { // FTP不可达
			...
			userInfo = uib.getUserName() + "," + uib.getUserID() + ","
					...
					+ uib.getFtpPasswd() + "," + Result + "," + uib.getLocalPath();
			for (int i = 0; i < num; i++) {
				if (icqPop.infomap.get(i).contains(uib.getUserName() + "," + uib.getUserID())) {
					icqPop.infomap.remove(i);
					icqPop.infomap.put(i, userInfo);
					System.out.println("FTP不可达userInfo:" + i + "||" + userInfo);
					break;
				}
			}
			return;
		...
		if (LoadResult == 1) { // 在线
			...
			for (int i = 0; i < num; i++) {
				if (icqPop.infomap.get(i).contains(uib.getUserName() + "," + uib.getUserID())) {
					icqPop.infomap.remove(i);
					icqPop.infomap.put(i, userInfo);
					System.out.println("在线userInfo:" + i + "||" + userInfo);
					break;
				}
			}
			...
			for (int i = 0; i < num; i++) {
				if (icqPop.infomap.get(i).contains(uib.getUserName() + "," + uib.getUserID())) {
					icqPop.infomap.remove(i);
					icqPop.infomap.put(i, userInfo);
					System.out.println("离线，有文件userInfo:" + i + "||" + userInfo);
					break;
				}
			}
			...
			for (int i = 0; i < num; i++) {
				if (icqPop.infomap.get(i).contains(uib.getUserName() + "," + uib.getUserID())) {
					icqPop.infomap.remove(i);
					icqPop.infomap.put(i, userInfo);
					System.out.println("离线，没文件userInfo:" + i + "||" + userInfo);
					break;
				}
			}
			...
			for (int i = 0; i < num; i++) {
				if (icqPop.infomap.get(i).contains(uib.getUserName() + "," + uib.getUserID())) {
					icqPop.infomap.remove(i);
					icqPop.infomap.put(i, userInfo);
					System.out.println("软件已被卸载userInfo:" + i + "||" + userInfo);
					break;
				}
			}
		}
	...
	private int loadFile(String path) {
		...
		System.out.println("上次记录的时间=" + Month + "-" + Day + "-" + Hour + "-"
				+ Minute);
		lastTime = (Month - 1) * 30 * 24 * 60 + (Day - 1) * 24 * 60
				+ (Hour - 0) * 60 + Minute;
		...
		if (listfile.contains("active.result")) {
			...
			System.out.println("读取到的时间=" + month + "-" + day + "-" + hour + "-"
					+ minute);
			currentTime = (month - 1) * 30 * 24 * 60 + (day - 1) * 24 * 60
					+ (hour - 0) * 60 + minute;
			...
			if (state == 1) {
				if (lastTime == currentTime) {
					int temp = icqPop.nummap.get(uib.getUserID());
					if (temp >= 3) {
						System.err.println("超时次数超过3次！下线处理。");
						if (check) {
							return -2;
						} else
							return 0;

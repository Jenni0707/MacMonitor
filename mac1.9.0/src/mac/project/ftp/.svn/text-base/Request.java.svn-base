...
public class Request {
	...
	private void parse() {
		...
		if (index <0) cmd = request;
			else cmd = request.substring(0,index);
		if (cmd.equals("USER")) {
	...
	private int parsePort(String portCmd) {
		...
		int low = Integer.parseInt(portStr.substring(index+1,portStr.length()));
	...
	public String getMsg() {
	    ...
	    System.out.print(requestBuf.toString());
